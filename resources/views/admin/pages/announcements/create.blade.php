...
@section('content')
    {{--
        نفس بنية صفحات النماذج الموحّدة
        (انظر admin/pages/company-bank-accounts/create.blade.php):
        admin-page-shell ← admin-page-banner ← admin-page-card
        ← form.admin-form ← admin-form-body ← admin-form-footer
    --}}
            ...
            <div class="admin-page-banner">
                <div class="admin-page-banner-main">
                    ...
                    <div class="admin-page-banner-text">
                        ...
                        <p>اكتب الإعلان وحدّد حالته واستهدافه ومواعيد نشره وانتهائه</p>
                    </div>
                </div>
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="status" name="status" required
                                    ...
                                    <option value="draft" @selected(old('status', 'draft') === 'draft')>مسودة</option>
                            ...
                            <div class="col-md-6">
                                ...
                                <select id="target_type" name="target_type" required
                                    ...
                                    <option value="all" @selected(old('target_type', 'all') === 'all')>الجميع</option>
                                    <option value="department" @selected(old('target_type') === 'department')>قسم محدد</option>
                                    <option value="branch" @selected(old('target_type') === 'branch')>فرع محدد</option>
                                </select>
                                @error('target_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6" id="wrap_department" @if(old('target_type') !== 'department') hidden @endif>
                            ...
                            <div class="col-md-6" id="wrap_branch" @if(old('target_type') !== 'branch') hidden @endif>
                                <label class="admin-form-label" for="branch_id">الفرع</label>
                                <select id="branch_id" name="branch_id" class="form-select">
                                    <option value="">— اختر الفرع —</option>
                                    @foreach ($branches as $br)
                                        <option value="{{ $br->id }}" @selected(old('branch_id') == $br->id)>
                                            {{ $br->name_ar ?? $br->name }}
                                        </option>
                                    @endforeach
                                </select>
