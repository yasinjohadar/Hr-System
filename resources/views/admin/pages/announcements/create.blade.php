    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">إضافة إعلان جديد</h5>
                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.announcements.store') }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <textarea class="form-control @error('content') is-invalid @enderror" name="content"
                                          rows="6" placeholder="نص الإعلان">{{ old('content') }}</textarea>
                                @error('content')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="target_type" id="target_type" class="form-select @error('target_type') is-invalid @enderror" required>
                                    <option value="all" {{ old('target_type', 'all') == 'all' ? 'selected' : '' }}>الجميع</option>
                                    <option value="department" {{ old('target_type') == 'department' ? 'selected' : '' }}>قسم محدد</option>
                                    <option value="branch" {{ old('target_type') == 'branch' ? 'selected' : '' }}>فرع محدد</option>
                                </select>
                                @error('target_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6" id="wrap_department" style="{{ old('target_type') == 'department' ? '' : 'display:none' }}">
                                <label class="form-label">القسم</label>
                                <select name="department_id" class="form-select">
                                    <option value="">-- اختر القسم --</option>
                                    @foreach ($departments as $dept)
                                        <option value="{{ $dept->id }}" {{ old('department_id') == $dept->id ? 'selected' : '' }}>{{ $dept->name_ar ?? $dept->name }}</option>
                                    @endforeach
                                </select>
                            </div>

                            <div class="col-md-6" id="wrap_branch" style="{{ old('target_type') == 'branch' ? '' : 'display:none' }}">
                                <label class="form-label">الفرع</label>
                                <select name="branch_id" class="form-select">
                                    <option value="">-- اختر الفرع --</option>
                                    @foreach ($branches as $br)
                                        <option value="{{ $br->id }}" {{ old('branch_id') == $br->id ? 'selected' : '' }}>{{ $br->name_ar ?? $br->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                        ...
                        <div class="text-end mt-4">
                            ...
                            <button type="submit" class="btn btn-primary px-4"><i class="fas fa-save me-2"></i>حفظ الإعلان</button>
                        </div>
                    </form>
                </div>
            </div>
