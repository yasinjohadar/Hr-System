    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div class="my-auto">
                    <h5 class="page-title fs-21 mb-1">تعديل الصيانة</h5>
                </div>
                <div>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.asset-maintenances.update', $maintenance->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="maintenance_type" class="form-select @error('maintenance_type') is-invalid @enderror" required>
                                    <option value="preventive" {{ old('maintenance_type', $maintenance->maintenance_type) == 'preventive' ? 'selected' : '' }}>وقائية</option>
                                    <option value="corrective" {{ old('maintenance_type', $maintenance->maintenance_type) == 'corrective' ? 'selected' : '' }}>تصحيحية</option>
                                    <option value="upgrade" {{ old('maintenance_type', $maintenance->maintenance_type) == 'upgrade' ? 'selected' : '' }}>ترقية</option>
                                    <option value="cleaning" {{ old('maintenance_type', $maintenance->maintenance_type) == 'cleaning' ? 'selected' : '' }}>تنظيف</option>
                                    <option value="inspection" {{ old('maintenance_type', $maintenance->maintenance_type) == 'inspection' ? 'selected' : '' }}>فحص</option>
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                                    <option value="scheduled" {{ old('status', $maintenance->status) == 'scheduled' ? 'selected' : '' }}>مجدولة</option>
                                    ...
                                    <option value="cancelled" {{ old('status', $maintenance->status) == 'cancelled' ? 'selected' : '' }}>ملغاة</option>
                                    <option value="postponed" {{ old('status', $maintenance->status) == 'postponed' ? 'selected' : '' }}>مؤجلة</option>
                                </select>
                                @error('status')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="actual_date" class="form-control" 
                                       value="{{ old('actual_date', $maintenance->actual_date?->format('Y-m-d')) }}">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="service_provider" class="form-control" 
                                       value="{{ old('service_provider', $maintenance->service_provider) }}">
                            </div>

                            <div class="col-md-6">
                                <label class="form-label">معلومات الاتصال بمزود الخدمة</label>
                                <input type="text" name="service_provider_contact" class="form-control" 
                                       value="{{ old('service_provider_contact', $maintenance->service_provider_contact) }}">
                            </div>
