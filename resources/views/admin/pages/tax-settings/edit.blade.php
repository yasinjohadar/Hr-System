    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">تعديل إعداد الضريبة</h5>
                <a href="{{ route('admin.tax-settings.index') }}" class="btn btn-secondary btn-sm">رجوع</a>
            </div>

            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.tax-settings.update', $taxSetting->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" name="type" required>
                                    ...
                                    <option value="income_tax" {{ old('type', $taxSetting->type) == 'income_tax' ? 'selected' : '' }}>ضريبة الدخل</option>
                                    <option value="social_insurance" {{ old('type', $taxSetting->type) == 'social_insurance' ? 'selected' : '' }}>التأمينات الاجتماعية</option>
                                    <option value="health_insurance" {{ old('type', $taxSetting->type) == 'health_insurance' ? 'selected' : '' }}>التأمين الصحي</option>
                                    <option value="other" {{ old('type', $taxSetting->type) == 'other' ? 'selected' : '' }}>أخرى</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('calculation_method') is-invalid @enderror" name="calculation_method" required id="calculation_method">
                                    ...
                                    <option value="percentage" {{ old('calculation_method', $taxSetting->calculation_method) == 'percentage' ? 'selected' : '' }}>نسبة مئوية</option>
                                    <option value="slab" {{ old('calculation_method', $taxSetting->calculation_method) == 'slab' ? 'selected' : '' }}>شرائح</option>
                                    <option value="fixed" {{ old('calculation_method', $taxSetting->calculation_method) == 'fixed' ? 'selected' : '' }}>ثابت</option>
                                </select>
                            ...
                            <div class="col-md-6">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $taxSetting->is_active) ? 'checked' : '' }}>
                                        ...
                                        <div class="slab-item mb-3 p-3 border rounded">
                                            <div class="row g-3">
                                                ...
                                                <div class="col-md-1">
                                                    ...
                                                    <button type="button" class="btn btn-danger btn-sm remove-slab" {{ count($taxSetting->slabs) == 1 ? 'style="display:none;"' : '' }}>حذف</button>
                                                </div>
                                            </div>
                                        </div>
                                    @endforeach
