    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">تعديل الحدث</h5>
                <a href="{{ route('admin.calendar-events.index') }}" class="btn btn-secondary btn-sm">رجوع</a>
            </div>

            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.calendar-events.update', $event->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" class="form-control" name="title_ar" value="{{ old('title_ar', $event->title_ar) }}">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('type') is-invalid @enderror" name="type" required id="event_type">
                                    ...
                                    <option value="personal" {{ old('type', $event->type) == 'personal' ? 'selected' : '' }}>شخصي (للموظف الحالي)</option>
                                    <option value="public" {{ old('type', $event->type) == 'public' ? 'selected' : '' }}>عام (للمؤسسة)</option>
                                    <option value="all" {{ old('type', $event->type) == 'all' ? 'selected' : '' }}>للجميع</option>
                                    <option value="department" {{ old('type', $event->type) == 'department' ? 'selected' : '' }}>لقسم معين</option>
                                    <option value="employee" {{ old('type', $event->type) == 'employee' ? 'selected' : '' }}>لموظف معين</option>
                                </select>
                                @error('type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6" id="department_div" style="display: none;">
                                ...
                                <select class="form-select" name="department_id" id="department_id">
                                    ...
                                    @foreach ($departments as $dept)
                                        <option value="{{ $dept->id }}" {{ old('department_id', $event->department_id) == $dept->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6" id="employee_div" style="display: none;">
                                ...
                                <select class="form-select" name="employee_id" id="employee_id">
                                    ...
                                    @foreach ($employees as $emp)
                                        <option value="{{ $emp->id }}" {{ old('employee_id', $event->employee_id) == $emp->id ? 'selected' : '' }}>
                                            {{ $emp->full_name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="datetime-local" class="form-control @error('end_date') is-invalid @enderror" 
                                       name="end_date" value="{{ old('end_date', $event->end_date?->format('Y-m-d\TH:i')) }}">
                                @error('end_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="color" class="form-control form-control-color" name="color" value="{{ old('color', $event->color) }}">
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-check mt-4">
                                    <input class="form-check-input" type="checkbox" name="is_reminder" id="is_reminder" value="1" {{ old('is_reminder', $event->is_reminder) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-6" id="reminder_minutes_div" style="display: none;">
                                ...
                                <input type="number" class="form-control" name="reminder_minutes" value="{{ old('reminder_minutes', $event->reminder_minutes) }}" min="1">
                            </div>
