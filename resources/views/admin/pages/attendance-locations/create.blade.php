    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">إضافة موقع حضور جديد</h5>
                    ...
                    <form method="POST" action="{{ route('admin.attendance-locations.store') }}">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <small class="text-muted">سيتم إنشاؤه تلقائياً إذا لم يتم تحديده</small>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.00000001" class="form-control @error('latitude') is-invalid @enderror" 
                                       name="latitude" value="{{ old('latitude') }}" required min="-90" max="90">
                                @error('latitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">مثال: 24.7136</small>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" step="0.00000001" class="form-control @error('longitude') is-invalid @enderror" 
                                       name="longitude" value="{{ old('longitude') }}" required min="-180" max="180">
                                @error('longitude')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">مثال: 46.6753</small>
                            </div>
                            ...
                            <div class="col-md-4">
                                ...
                                <input type="number" class="form-control @error('radius_meters') is-invalid @enderror" 
                                       name="radius_meters" value="{{ old('radius_meters', 100) }}" required min="10" max="10000">
                                @error('radius_meters')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">المسافة المسموحة من الموقع (10-10000 متر)</small>
                            </div>
                            ...
                            <div class="col-md-6">
                                <div class="form-check mt-4">
                                    <input class="form-check-input" type="checkbox" name="require_location" id="require_location" value="1" {{ old('require_location', true) ? 'checked' : '' }}>
                            ...
                            <div class="col-md-12">
                                ...
                                <select class="form-select" name="allowed_employees[]" multiple size="5">
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ in_array($employee->id, old('allowed_employees', [])) ? 'selected' : '' }}>
                                            {{ $employee->full_name }} - {{ $employee->employee_number }}
                                        </option>
                                    @endforeach
                                </select>
                                <small class="text-muted">اتركه فارغاً للسماح للجميع. اضغط Ctrl/Cmd لاختيار متعدد</small>
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="allowed_departments[]" multiple size="5">
                                    @foreach ($departments as $department)
                                        <option value="{{ $department->id }}" {{ in_array($department->id, old('allowed_departments', [])) ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="allowed_positions[]" multiple size="5">
                                    @foreach ($positions as $position)
                                        <option value="{{ $position->id }}" {{ in_array($position->id, old('allowed_positions', [])) ? 'selected' : '' }}>
