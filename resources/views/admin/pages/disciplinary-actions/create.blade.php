    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div class="my-auto">
                    <h5 class="page-title fs-21 mb-1">إضافة إجراء تأديبي جديد</h5>
                </div>
                <div>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.disciplinary-actions.store') }}">
                        @csrf
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="action_type" class="form-select @error('action_type') is-invalid @enderror" required id="action_type">
                                    ...
                                    <option value="verbal_warning" {{ old('action_type') == 'verbal_warning' ? 'selected' : '' }}>تحذير شفهي</option>
                                    <option value="written_warning" {{ old('action_type') == 'written_warning' ? 'selected' : '' }}>تحذير كتابي</option>
                                    <option value="final_warning" {{ old('action_type') == 'final_warning' ? 'selected' : '' }}>إنذار نهائي</option>
                                    <option value="deduction" {{ old('action_type') == 'deduction' ? 'selected' : '' }}>خصم</option>
                                    <option value="suspension" {{ old('action_type') == 'suspension' ? 'selected' : '' }}>إيقاف</option>
                                    <option value="termination" {{ old('action_type') == 'termination' ? 'selected' : '' }}>إنهاء خدمة</option>
                                </select>
                                @error('action_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <select name="severity_level" class="form-select @error('severity_level') is-invalid @enderror" required>
                                    <option value="1" {{ old('severity_level', 1) == 1 ? 'selected' : '' }}>منخفض</option>
                                    <option value="2" {{ old('severity_level') == 2 ? 'selected' : '' }}>متوسط</option>
                                    <option value="3" {{ old('severity_level') == 3 ? 'selected' : '' }}>عالي</option>
                                    <option value="4" {{ old('severity_level') == 4 ? 'selected' : '' }}>عالي جداً</option>
                                    <option value="5" {{ old('severity_level') == 5 ? 'selected' : '' }}>حرج</option>
                                </select>
