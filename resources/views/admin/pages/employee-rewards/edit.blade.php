    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="d-md-flex d-block align-items-center justify-content-between my-4 page-header-breadcrumb">
                <div class="my-auto">
                    <h5 class="page-title fs-21 mb-1">تعديل مكافأة الموظف</h5>
                </div>
                <div>
            ...
            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.employee-rewards.update', $reward->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('employee_id') is-invalid @enderror" 
                                    ...
                                    @foreach ($employees as $employee)
                                        <option value="{{ $employee->id }}" {{ old('employee_id', $reward->employee_id) == $employee->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('reward_type_id') is-invalid @enderror" 
                                    ...
                                    @foreach ($rewardTypes as $rewardType)
                                        <option value="{{ $rewardType->id }}" {{ old('reward_type_id', $reward->reward_type_id) == $rewardType->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="date" name="reward_date" class="form-control @error('reward_date') is-invalid @enderror" 
                                       value="{{ old('reward_date', $reward->reward_date->format('Y-m-d')) }}" required>
                                @error('reward_date')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('reason') is-invalid @enderror" 
                                    ...
                                    <option value="performance" {{ old('reason', $reward->reason) == 'performance' ? 'selected' : '' }}>أداء</option>
                                    <option value="achievement" {{ old('reason', $reward->reason) == 'achievement' ? 'selected' : '' }}>إنجاز</option>
                                    <option value="milestone" {{ old('reason', $reward->reason) == 'milestone' ? 'selected' : '' }}>معلم</option>
                                    <option value="recognition" {{ old('reason', $reward->reason) == 'recognition' ? 'selected' : '' }}>اعتراف</option>
                                    <option value="other" {{ old('reason', $reward->reason) == 'other' ? 'selected' : '' }}>أخرى</option>
                                </select>
                                @error('reason')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" step="0.01" name="monetary_value" class="form-control" 
                                       value="{{ old('monetary_value', $reward->monetary_value) }}" min="0">
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select" name="currency_id">
                                    ...
                                    @foreach ($currencies as $currency)
                                        <option value="{{ $currency->id }}" {{ old('currency_id', $reward->currency_id) == $currency->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="number" name="points" class="form-control" 
                                       value="{{ old('points', $reward->points) }}" min="0">
                            </div>
                            ...
                            <div class="col-md-12">
                                ...
                                <textarea name="notes" class="form-control" rows="3">{{ old('notes', $reward->notes) }}</textarea>
                            </div>
                        </div>
