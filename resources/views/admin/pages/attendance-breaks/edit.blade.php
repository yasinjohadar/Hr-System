    ...
    <div class="main-content app-content">
        <div class="container-fluid">
            <div class="page-header d-flex justify-content-between align-items-center my-4">
                <h5 class="page-title mb-0">تعديل الاستراحة</h5>
                <a href="{{ route('admin.attendance-breaks.index') }}" class="btn btn-secondary btn-sm">رجوع</a>
            </div>

            <div class="card">
                <div class="card-body">
                    <form method="POST" action="{{ route('admin.attendance-breaks.update', $break->id) }}">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('attendance_id') is-invalid @enderror" name="attendance_id" required>
                                    <option value="">اختر سجل الحضور</option>
                                    @foreach ($attendances as $att)
                                        <option value="{{ $att->id }}" {{ old('attendance_id', $break->attendance_id) == $att->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-6">
                                ...
                                <select class="form-select @error('break_type') is-invalid @enderror" name="break_type" required>
                                    ...
                                    <option value="lunch" {{ old('break_type', $break->break_type) == 'lunch' ? 'selected' : '' }}>غداء</option>
                                    <option value="coffee" {{ old('break_type', $break->break_type) == 'coffee' ? 'selected' : '' }}>قهوة</option>
                                    <option value="prayer" {{ old('break_type', $break->break_type) == 'prayer' ? 'selected' : '' }}>صلاة</option>
                                    <option value="other" {{ old('break_type', $break->break_type) == 'other' ? 'selected' : '' }}>أخرى</option>
                                </select>
                                @error('break_type')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control @error('break_start') is-invalid @enderror" 
                                       name="break_start" value="{{ old('break_start', $break->break_start) }}" required>
                                @error('break_start')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="time" class="form-control @error('break_end') is-invalid @enderror" 
                                       name="break_end" value="{{ old('break_end', $break->break_end) }}" id="break_end">
                                @error('break_end')
                                    <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                                <small class="text-muted">المدة الحالية: {{ $break->duration_minutes }} دقيقة</small>
                            </div>

                            <div class="col-md-12">
                                <label class="form-label">ملاحظات</label>
                                <textarea class="form-control" name="notes" rows="3">{{ old('notes', $break->notes) }}</textarea>
                            </div>
                        </div>
